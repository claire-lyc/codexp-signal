...
export const volunteerNeedsStorageVersionKey = 'signal.volunteer.needs.version';
export const volunteerNotificationsStorageKey = 'signal.volunteer.notifications.v1';
const currentVolunteerNeedsStorageVersion = '2';
...
export function saveCustomVolunteerOpportunities(opportunities: VolunteerOpportunity[]) {
  if (typeof window === 'undefined') return;
  window.localStorage.setItem(volunteerNeedsStorageVersionKey, currentVolunteerNeedsStorageVersion);
...
export function clearStaleVolunteerNeeds() {
  if (typeof window === 'undefined') return;
  if (window.localStorage.getItem(volunteerNeedsStorageVersionKey) === currentVolunteerNeedsStorageVersion) return;
  window.localStorage.removeItem(volunteerNeedsStorageKey);
  window.localStorage.setItem(volunteerNeedsStorageVersionKey, currentVolunteerNeedsStorageVersion);
}
