...
import { AlertCircle, ArrowLeft, Bell, LogOut, Save, Shield, UserCircle } from 'lucide-react';
...
export default function GovProfile() {
  ...
  const agencyName = user?.agencyCode ? agencyNames[user.agencyCode] ?? user.agencyCode : null;
  const role = user?.role && user.role !== user.agencyCode ? user.role : null;
  const profileName = user?.displayName ?? user?.username ?? user?.email ?? 'Government User';
  const profileSubtitle = `${agencyName ?? 'Government agency'}${role ? ` - ${role}` : ''}`;
  const profileTags = user?.tags?.length ? user.tags.map((tag) => agencyNames[tag] ?? tag) : [agencyName ?? role ?? 'Government agency'];

  return (
    <div className="min-h-screen bg-zinc-950 px-6 py-8 text-white">
      <div className="mx-auto max-w-6xl">
        <Link to="/gov" className="mb-6 inline-flex items-center gap-2 text-sm text-zinc-400 hover:text-white">
          <ArrowLeft className="h-4 w-4" />
          Back to command centre
        </Link>

        <div className="grid gap-8 lg:grid-cols-[260px_minmax(0,1fr)]">
          <aside className="rounded-xl border border-zinc-800 bg-zinc-900 p-3 lg:sticky lg:top-8 lg:self-start">
          ...
          <main className="min-w-0 space-y-6">
            <div>
              <h1 className="text-3xl font-bold">Government Account</h1>
              <p className="mt-1 text-zinc-400">Manage operator details, notifications, and sign-out.</p>
            </div>
              ...
              <section className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
                ...
                <div className="mb-5 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-zinc-500">{user?.email ?? error ?? profileSubtitle}</div>
                  </div>
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <InfoRow label="User" value={user?.username ?? user?.email ?? 'Not signed in'} />
                  <InfoRow label="Email" value={user?.email ?? 'Not set'} />
                  <InfoRow label="Role" value={role ?? 'Government user'} />
